...
const MapAutocomplete = () => {
  ...
  return <div id="gmp-map" style={{ height: '400px', width: '100%' }}></div>;
};
  ...
  useEffect(() => {
    ...
    function handleAutocomplete(place) {
      const addressData = {
        ...
        administrative_area_level_1: place.address_components.find(comp => comp.types.includes('administrative_area_level_1'))?.short_name || '',
        country: place.address_components.find(comp => comp.types.includes('country'))?.long_name || '',
        postal_code: place.address_components.find(comp => comp.types.includes('postal_code'))?.short_name || '',
      };
