    ...
    const fetchAndSortData = async () => {
      try {
        ...
        for (const task of filteredData) {
          ...
          if (endTime <= new Date(endTime.getFullYear(), endTime.getMonth(), endTime.getDate(), 6, 0, 0, 0)) {
          ...
          const isOverlap = calendarEvents.some((event) => {
            const eventStart = new Date(event.start.dateTime);
            const eventEnd = new Date(event.end.dateTime);

            return startTime < eventEnd && endTime > eventStart; // Check for overlap
          });
